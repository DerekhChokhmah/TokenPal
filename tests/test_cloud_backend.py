...
def test_synthesize_haiku_does_not_send_thinking(fake_anthropic: dict[str, Any]) -> None:
...
def test_synthesize_sonnet_enables_adaptive_thinking(fake_anthropic: dict[str, Any]) -> None:
    client = _FakeClient(result=_fake_message("{}"))
    fake_anthropic["client"] = client
...
def test_synthesize_opus_enables_adaptive_thinking(fake_anthropic: dict[str, Any]) -> None:
    client = _FakeClient(result=_fake_message("{}"))
    fake_anthropic["client"] = client
...
def test_research_deep_sends_web_search_and_web_fetch_tools(fake_anthropic: dict[str, Any]) -> None:
...
def test_research_deep_continues_on_pause_turn(fake_anthropic: dict[str, Any]) -> None:
...
def test_research_deep_caps_continuations(fake_anthropic: dict[str, Any]) -> None:
...
def test_research_deep_search_only_omits_web_fetch(fake_anthropic: dict[str, Any]) -> None:
...
def test_research_deep_auth_error_surfaces_kind(fake_anthropic: dict[str, Any]) -> None:
    import anthropic
...
def test_apply_cache_breakpoint_strips_existing_markers() -> None:
    ...
    assert all("cache_control" not in b for b in out[1]["content"] if isinstance(b, dict))
...
def test_apply_cache_breakpoint_stays_within_4_limit_over_many_followups() -> None:
    ...
    for i in range(6):
        ...
        assert cc_count == 1, f"round {i}: expected 1 cache_control, got {cc_count}"
...
def test_followup_synth_happy_path(fake_anthropic: dict[str, Any]) -> None:
    ...
    assert result.messages[2] == {
        "role": "user",
        "content": "I already tried X, what else?",
    }


def test_followup_sends_cache_control_on_prior_assistant(fake_anthropic: dict[str, Any]) -> None:
...
def test_followup_enable_cache_false_skips_breakpoint(fake_anthropic: dict[str, Any]) -> None:
    client = _FakeClient(result=_fake_message("ok"))
    fake_anthropic["client"] = client
    b = CloudBackend(api_key="sk-ant-test", model="claude-haiku-4-5")
    prior = [
...
def test_followup_forwards_tools_when_search_session(fake_anthropic: dict[str, Any]) -> None:
    client = _FakeClient(result=_fake_message("ok"))
    fake_anthropic["client"] = client
    b = CloudBackend(api_key="sk-ant-test", model="claude-sonnet-4-6")
    prior = [
...
def test_followup_omits_tools_for_synth_session(fake_anthropic: dict[str, Any]) -> None:
    client = _FakeClient(result=_fake_message("ok"))
    fake_anthropic["client"] = client
    b = CloudBackend(api_key="sk-ant-test", model="claude-haiku-4-5")
    b.followup(
...
def test_followup_auth_error_raises_cloud_backend_error(fake_anthropic: dict[str, Any]) -> None:
    import anthropic

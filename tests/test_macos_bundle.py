...
def test_ensure_bundle_builds_the_tree_and_a_readable_info_plist(
    ...
    site_target = os.path.normpath(str(site.readlink())).removeprefix("\\\\?\\")
    expected_target = os.path.normpath(str(fake_env))
    assert os.path.normcase(site_target) == os.path.normcase(expected_target)
...
def test_ensure_bundle_replaces_a_stampless_bundle(
    ...
    stub_path = Path(stamp["stub"])
    assert stub_path.name == "Python"
    assert stub_path.parent.name == "MacOS"
    assert stub_path.parent.parent.name == "Contents"
    assert stub_path.parent.parent.parent.name == "Python.app"
...
def test_would_use_bundle_follows_the_resolved_overlay(
    ...
    discover_overlays()
    monkeypatch.setattr(macos_bundle, "pyobjc_available", lambda: True)
